...
clf = SVC(kernel='linear', probability=True, tol=1e-3)  #Establece el clasificador como un vector de apoyo de máquinas

def get_files(emotion):
    files = glob.glob("dataset\\%s\\*" %emotion)
    random.shuffle(files)
    training = files[:int(len(files))] 
    prediction=['dataset\\yo.png'] #aqui se añade la imagen que quiere procesar
...
frame=cv2.imread('dataset\\yo.png') #aqui se añade la imagen que quieres procesar pero aqui solo se carga para el resultado final
